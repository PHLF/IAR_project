...
class EvOverseer
{
  ...
  private:
    ...
    toml::table _settings{
        {{"simulation",
          toml::table{{{"threads", 1},
                       ...
                       {"universe",
                        toml::table{{{"width", 512}, {"height", 512}, {"closed curvature", false}}}}

          }}},
         {"viewport", toml::table{{{"headless", true}, {"width", 768}, {"height", 768}}}},
         {"predator", toml::table{{{"number", 1},
                                   {"speed", 1},
                                   {"turn rate", 8},
                                   {"confusion", false},
         ...
         {"genome mutation",
          toml::table{{{"per site probability", toml::table{{
                       ...
                       {"per gene probability", toml::table{{{"duplication", 0.005},
                                                             {"deletion", 0.010},
    ...
    OptSimResult _run_thread(uint32_t thread_number, uint32_t generation,
