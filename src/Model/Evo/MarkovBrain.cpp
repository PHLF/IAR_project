...
void MarkovBrain::_instantiate_plg(uint32_t index)
{
    ...
    const auto increase_step = [&](uint32_t step) -> void {
        ...
        {
            if (auto existing_plg =
                    std::find(std::cbegin(_plg_indexes), std::cend(_plg_indexes), index);
    ...
    nb_inputs  = static_cast<uint32_t>(std::floor(current_symbol / 255.0 * _max_inputs));
    nb_inputs  = nb_inputs > 0 ? nb_inputs : 1;
    nb_outputs = static_cast<uint32_t>(std::floor(next_symbol / 255.0 * _max_outputs));
    ...
    for (uint32_t i = 0; i < _max_inputs; ++i)
    {
        if (input_nodes_ids.size() < static_cast<uint64_t>(nb_inputs))
        {
            input_nodes_ids.emplace_back(
                std::round(static_cast<double>(current_symbol * (_nb_nodes - 1)) / 255));
        }
    ...
    table.shrink_to_fit();
    _prob_logic_gates.emplace_back(ProbabilisticLogicGate(
        std::move(input_nodes_ids), std::move(output_nodes_ids), std::move(table)));
}
...
void MarkovBrain::mutation(const toml::table& mutations_proba)
{
    ...
    const auto proba_site_copy =
        mutations_proba["per site probability"]["copy"].as_floating_point()->get() * mutation_rate;
    const auto proba_site_del =
        mutations_proba["per site probability"]["deletion"].as_floating_point()->get() *
        mutation_rate;
    const auto proba_site_insert =
        mutations_proba["per site probability"]["insertion"].as_floating_point()->get() *
        mutation_rate;
    const auto proba_site_replaced =
        mutations_proba["per site probability"]["substitution"].as_floating_point()->get() *
        mutation_rate;
    const auto proba_site_gauss_mut =
        mutations_proba["per site probability"]["gaussian mutation"].as_floating_point()->get() *
        mutation_rate;
    const auto proba_gene_duplication =
        mutations_proba["per gene probability"]["duplication"].as_floating_point()->get() *
        mutation_rate;
    const auto proba_gene_deletion =
        mutations_proba["per gene probability"]["deletion"].as_floating_point()->get() *
        mutation_rate;
    const auto proba_new_gene_insert =
        mutations_proba["per gene probability"]["new gene insertion"].as_floating_point()->get() *
        mutation_rate;
...
void MarkovBrain::_site_gaussian_mutation()
{
    ...
    tmp = static_cast<uint8_t>(
        byte_gaussian_dist(_gen, param{static_cast<double>(_genome[index]), sigma}));
    _genome[index] = tmp;
}
...
void MarkovBrain::_site_insert_mutation()
{
    ...
    uint64_t index = rand_index(_gen);
    _genome.insert(std::begin(_genome) + static_cast<int64_t>(index), d_uni(_gen));
}
...
void MarkovBrain::_gene_insert_mutation()
{
    ...
    _genome.insert(std::begin(_genome) + static_cast<int64_t>(index), std::begin(gene),
                   std::end(gene));
}
...
void MarkovBrain::_gene_delete_mutation()
{
    ...
    {
        std::uniform_int_distribution<uint64_t> rand_index_genes{0, genes_positions.size() - 2};
        ...
        _genome.erase(std::begin(_genome) + gene_start, std::begin(_genome) + gene_end);
    }
...
void MarkovBrain::_gene_duplication_mutation()
{
    ...
    {
        std::vector<uint8_t> duplicated_gene{std::begin(_genome), std::end(_genome)};
        _genome.insert(std::begin(_genome) + static_cast<int64_t>(index),
                       std::begin(duplicated_gene), std::end(duplicated_gene));
    }
}
...
std::vector<uint8_t> MarkovBrain::_build_gene()
{
    ...
    uint32_t const nb_inputs  = static_cast<uint32_t>(std::floor(gene[2] / 255.0 * _max_inputs));
    uint32_t const nb_outputs = static_cast<uint32_t>(std::floor(gene[3] / 255.0 * _max_outputs));
